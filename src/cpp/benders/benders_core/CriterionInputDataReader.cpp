...
double CriterionSingleInputData::Criterion() const
{
    return criterion_;
}
...
std::string CriterionInputData::PatternsPrefix() const
{
    ...
    {
        ret = StringManip::split(criterion_vector_[0].Pattern().GetPrefix(), "::")[0];
    }
    return ret;
}
...
OuterLoopCriterionInputData CriterionInputFromYaml::Read(const std::filesystem::path& input_file)
{
    ...
    catch (const std::exception& e)
    {
        ...
        throw CriterionInputFileError(PrefixMessage(LogUtils::LOGLEVEL::FATAL, "Outer Loop"),
                                      err_msg.str(),
                                      LOGLOCATION);
    }
    if (yaml_content.IsNull())
    {
        ...
        throw CriterionInputFileIsEmpty(PrefixMessage(LogUtils::LOGLEVEL::FATAL, "Outer Loop"),
                                        err_msg.str(),
                                        LOGLOCATION);
    }
...
void OuterLoopCriterionInputData::setStoppingThreshold(double stoppingThreshold)
{
...
public:
    ...
    static bool decode(const Node& node, OuterLoopCriterionInputData& rhs)
    {
        ...
        rhs.SetCriterionCountThreshold(node["criterion_count_threshold"].as<double>(1));
